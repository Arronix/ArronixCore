...
public class HttpResponse<T>(HttpResponse response) : HttpResponse(response.Request, response.Headers, response.ResponseData, response.StatusCode, response.Version)
    where T : new()
{
    public T Resource { get; private set; } = Json.Deserialize<T>(response.Content);
}
