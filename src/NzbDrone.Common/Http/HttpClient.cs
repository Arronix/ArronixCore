...
public class HttpClient(IEnumerable<IHttpRequestInterceptor> requestInterceptors,
    ICacheManager cacheManager,
    IRateLimitService rateLimitService,
    IHttpDispatcher httpDispatcher,
    Logger logger) : IHttpClient
{
    private const int MaxRedirects = 5;

    private readonly Logger _logger = logger;
    private readonly IRateLimitService _rateLimitService = rateLimitService;
    private readonly ICached<CookieContainer> _cookieContainerCache = cacheManager.GetCache<CookieContainer>(typeof(HttpClient));
    private readonly List<IHttpRequestInterceptor> _requestInterceptors = requestInterceptors.ToList();
    private readonly IHttpDispatcher _httpDispatcher = httpDispatcher;
