...
#pragma warning restore CA1031
    ...
    internal bool TryCheckPackage(InstalledPackage package, out CoreErrorCode code, out IReadOnlyList<string> defects)
    {
        ...
        // Listing a package's own folder is file-boundary work on a path that package controls, so it uses
        // the closed allowlist. Containing it here is what keeps one unreadable folder from ending the load
        // pass: this check runs before the per-package try, so an escaping failure would abort every package
        // not yet attempted rather than refusing the one whose folder cannot be read.
#pragma warning disable CA1031
