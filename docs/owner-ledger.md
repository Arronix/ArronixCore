...
| O-40 | Media item identity has one rule. `ICataloger` owns an item's identity in a semantic catalog namespace, independent of any implementation; `ICurator` may own its own list and entry identifiers but references items only through a cataloger's catalog identity and never supplies an authoritative item; Host alone assigns `MediaItemId`, if and when a catalog item is materialized into local library state; repeated fetches, aliases, redirects and converging natural keys resolve idempotently to one local `MediaItemId`; and no catalog reference carries a `ProviderId`, a configured definition or a CLR implementation type. | 2026-08-25 · current Claude Code conversation | **Implemented, and the allocation moment is now the ledger's.** `IMediaEntity` declares no key; `CatalogIdentity` is host state scoped by kind and level; `CatalogDispatcher` routes by scheme. Search, fetch and curated resolve are reads answering with `CatalogCandidate<TItem>`, and assignment is host-internal and idempotent over the identity, the identifier asked for and the rest the item states — so "if and when a catalog item is materialized into local library state" is what the code does rather than what it approximated. No public member allocates, and no take-in operation is claimed while the library row it belongs in a transaction with does not exist — which is also why this boundary must not be merged as the durable G07B vertical. Resolution and assignment are separate contracts, and projection holds only the reader. A merge still resolves the superseded reference and moves no library rows; nothing persists. Records: `docs/research/g04/media-item-identity.md`, `docs/research/g07/durable-movies-vertical.md` Q1. |
