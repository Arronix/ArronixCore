...
acceptance, in the same posture G07.2 was in before its review. §6 records what closing the browser leg
found and fixed, because it changed a claim G07.2 had already made.
...
original record (reproduced below) lists its corrections. This record was itself forward-ported: the server
half landed on an earlier base, ahead of G07.2 and the G07.3 lifecycle core, and everything in §§5–7 below
is new — built here, on top of the now-integrated base, to close the seam §6 of the original record left
open.
...
## 6. What closing this leg found: the payload panel had never actually run

The first attempt at the browser half timed out waiting for the installation report, on the *first*
navigation to `/contracts` — before any ShortFilm-specific code ran. The server log was clean; the page
itself never finished starting. `Arronix.Client.Composition.ArronixClientServiceCollectionExtensions`
registered `ContractPayloadLoader` with `services.TryAddSingleton<ContractPayloadLoader>()` — the
reflection-activated overload — but `ContractPayloadLoader`'s only constructor is `internal`, deliberately,
because it takes `HttpClient` and `MediaContractLoader` and nothing outside `Arronix.Client` builds one
