...
exception type — so "unfamiliar" is literal rather than a stand-in — and covers the full matrix: exhausted
memory and cancellation, each from the constructor where reflection wraps them and from the identifier
getter where they arrive direct. A module constructed before a failing identifier getter is still disposed before its
...
| 11 | Teardown contains an unfamiliar disposer failure and propagates a process-fatal or canceled one | `PackageTeardownContainmentTests` (3 cases) |
...
| Teardown catches without the containment policy | 2 fail: the process-fatal and canceled disposer cases; the unfamiliar-disposer case still passes, which is the point of the rule |
...
```text
projects=12 total=3056 enabled=2754 passed=2754 failed=0 skipped=302 inconclusive=0
cases=302 replacements=0 requiredTests=3 compileLogs=1 compileProjects=12 compileItems=285 boundSources=15
```

Against the G02 close (2,168 passed of 2,470, 302 skipped) this is +586 enabled cases. The registered skip
